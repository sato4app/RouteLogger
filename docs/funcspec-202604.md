...
- **動作:**
  ...
  4. プレビュー確認画面を表示
      ...
      - 閉じるボタン: カメラモードを終了
  5. 撮影直後に **direction=0・facing='forward'** でデフォルト保存（方向ボタン未操作のままアップロードされても null にならない）
  6. Forward/Backwardボタンをタップすると:
     - 現在の角度・facing設定で上書き保存
...
```javascript
{
    ...
    timestamp: string,     // ISO 8601形式
    direction: number,     // 角度（度数、-180〜+180、0=正面、正=右、負=左）デフォルト: 0
    facing: string,        // "forward" | "backward"  デフォルト: "forward"
    location: {
        lat: number,       // 緯度（小数点以下5桁）
        lng: number,       // 経度（小数点以下5桁）
        accuracy: number   // 精度
...
| ルート開始マーカー | ネイビー四角（RouteLoggerデータ） | トラック開始地点 |
| ルート終了マーカー | ネイビー矢印（RouteLoggerデータ） | トラック終了地点と進行方向 |

#### 3.7.3 軌跡・外部レイヤー表示色
| データ種別 | 色 | 備考 |
|-----------|-----|------|
| 通常GPS記録中のライン | `#000080`（ネイビー） | 幅4px、透明度0.7 |
| KMZインポート復元データのライン・マーカー | `#000080`（ネイビー） | |
| 外部KMZ/GeoJSONのライン | `#0055ff`（青） | 幅4px、透明度0.7 |
| 外部KMZ/GeoJSONのポイント | `#0055ff`（青） | circleMarker 半径6px |
...
```javascript
{
    ...
    direction: number,     // 角度（度数、-180〜+180）デフォルト: 0
    facing: string,        // "forward" | "backward"  デフォルト: "forward"
    location: {
        lat: number,
        lng: number,
...
| 2026-04-09 | RLog-v11 | 写真撮影直後にdirection=0・facing='forward'でデフォルト保存（nullデータ防止）、KMZインポート復元データの色を#00BFFFから#000080（ネイビー）に変更、外部KMZ/GeoJSONの表示色を#4682b4から#0055ff（青）に変更 |
